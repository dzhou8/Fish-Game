...
struct Player
{
	...
	string name;
	bool cards[SUIT_MAX * NUMBER_MAX];
	bool isHuman;
	int ID;
	int teamID;
	int numCards;
	...
	Player(string name, bool isHuman, Game* g)
	{
		this->name = name;
		for (int i = 0; i < SUIT_MAX * NUMBER_MAX; i++)
		{
	...
	//get half suits you have (warning: if you have NUM_MAX, this will still be included)
	...
	//get cards you can ask for in a half suit
...
struct Game
{
	...
	only really useful for thinkingPlayer
	...
	*/
	...
	void takeTurn()
	{
		...
		while (players[turnID]->numCards == 0) //rip ran out of cards
		{
		...
		players[turnID]->takeTurn(); //turnaction.first = id of player to ask, turnaction.second = card id to ask for
	...
	bool infinityWar() //return if one team is completely out of cards -> i.e. go to endgame, the other team must guess without any more information
	{
...
struct ThinkingPlayer : Player
{
	...
	void generateKnowledge()
	{
		...
		/*for (int i = 0; i < halfSuitKnowledge.size(); i++)
		{
			for (int j = 0; j < halfSuitKnowledge[i].size(); j++)
			{
				cout << halfSuitKnowledge[i][j] << " ";
			}
			cout << endl;
		}*/
		...
		for (int halfSuit = 0; halfSuit < SUIT_MAX * 2; halfSuit++)
		{
			...
			if (count == NUMBER_MAX / 2) //all cards in halfsuit have been accounted for
			{
				...
				/*for (int i = 0; i < 10; i++)
				{
					cout << uniquePlayers << endl;
					cout << ((halfSuit % 2 == 0) ? " Low " : " High ") << SUIT_NAME[halfSuit / 2] << endl;
					cout << "FIGURED SHIT OUT" << endl;
				}*/
			}
		}
	}
	...
	void takeTurn()
	{
		...
		for (int i = 0; i < cardLocationKnowledge.size(); i++)
		{
			if (cardLocationKnowledge[i].size() == 1 && cardLocationKnowledge[i][0] != ID)
			{
				cout << g->players[cardLocationKnowledge[i][0]]->name << " has the " << getCardString(i) << endl;
			}
		}*/
